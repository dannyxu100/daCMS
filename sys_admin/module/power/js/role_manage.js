...
function selectmenu(obj){
	...
	if(obj.checked){
		da.runDB("/sys_admin/module/power/action/menu2role_add_item.php",{
			prid: g_prid,
			pmid: pm_id
		...
		da.runDB("/sys_admin/module/power/action/menu2role_delete_item.php",{
			prid: g_prid,
			pmid: pm_id
...
function loadmenu2role(){
	da.runDB("/sys_admin/module/power/action/menu2role_get_list.php",{
		dataType: "json",
		prid: g_prid
...
function loadpower2role(){
	da.runDB("/sys_admin/module/power/action/power2role_get_list.php",{
		dataType: "json",
		prid: g_prid
...
function addu2r(){
	...
	daWin({
		width: 650,
		height: 500,
		url: "/sys_admin/module/power/plugin/select_user.htm?ismulti=true",
		back: function( res ){
			var suids = [];
			for(var k in res){
			...
			da.runDB("/sys_admin/module/power/action/user2role_add_list.php",{
				prid: g_prid,
				uids: suids.join(",")
			},function(res){
				if("FALSE" == res){
...
function deleteu2r(){
	...
	if( 0<suids.length ){
		confirm("确认删除选中的人员吗？",function(){
			da.runDB("/sys_admin/module/power/action/user2role_delete_list.php",{
				prid: g_prid,
				uids: suids.join(",")
			},function(res){
				if("FALSE" == res){
