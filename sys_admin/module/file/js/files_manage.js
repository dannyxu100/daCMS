...
function deletefile( dir, filename ){
	...
	function(){
		da.runDB("/sys_admin/module/file/action/file_delete_item.php", {
			dir: dir,
			filename: filename
...
function editfile( dir, filename, filetype ){
	daWin({
		...
		height: 600,
		title: "文件编辑",
		url: '/sys_admin/module/file/edit_file.php?dir='+ dir +'&filename='+ filename +'&filetype='+ filetype
	});
}
...
function loadlist(){
	...
	daTable({
		id: "tb_list",
		url: "/sys_admin/module/file/action/files_get_list.php",
		data: data1,
		//loading: false,
		// page: false,
		pageSize: 9999,
		...
		field: function( fld, val, row, ds ){
			switch(fld){
				case "f_name":
					...
					// else if( g_viewtype[row.f_type] ){
						// val = '<a href="'+ row.dir +'" target="_blank" >'+ val +'</a>';
					// }
