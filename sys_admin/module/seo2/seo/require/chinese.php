...
class Chinese {
	...
	function Chinese($SourceLang,$TargetLang) {
		...
		$this->config['TargetLang'] = $this->getcode($TargetLang);//echo 5;exit;
		$this->OpenTable();
	}
	...
	function OpenTable() {
		if ($this->config['SourceLang']=="GB2312") {
			...
			if ($this->config['TargetLang'] == "UTF8") {
				//echo '6'.$this->config['codetable_dir'].$this->config['GBtoUnicode_table'];exit;
				$tmp = $this->openfile(ROOT_PATH.$this->config['codetable_dir'].$this->config['GBtoUnicode_table']);
		...
		if ($this->config['SourceLang']=="BIG5") {
			if ($this->config['TargetLang'] == "UTF8") {
				$tmp = $this->openfile(ROOT_PATH.$this->config['codetable_dir'].$this->config['BIG5toUnicode_table']);
				$this->unicode_table = array();
				while(list($key,$value)=each($tmp)) {
					$this->unicode_table[hexdec(substr($value,0,6))]=substr($value,7,6);
				}
		...
		if ($this->config['SourceLang']=="UTF8") {
			if ($this->config['TargetLang'] == "GB2312") {
					//echo ROOT_PATH.$this->config['codetable_dir'].$this->config['GBtoUnicode_table'];exit;
				$tmp = $this->openfile(ROOT_PATH.$this->config['codetable_dir'].$this->config['GBtoUnicode_table']);
				//echo $tmp;exit;
				$this->unicode_table = array();
				while(list($key,$value)=each($tmp)) {
