...
function beforeRemove(treeId, treeNode) {
	...
	function(){
		da.runDB("/sys_admin/module/setting/item/action/itemtype_get_list.php",{			//检查是否拥有下级部门
			itpid: treeNode.id
		},
		function(res){debugger;
			if('FALSE'==res){
				da.runDB("/sys_admin/module/setting/item/action/itemtype_delete_item.php",{
				 itid: treeNode.id
				},
				function(res){
					if("FALSE"==res){
...
function beforeRename(treeId, treeNode, newName) {
	...
	if (newName.length == 0) {
		...
		da.runDB("/sys_admin/module/setting/item/action/itemtype_update_item.php",{
			itid: treeNode.id,
			itname: newName
		},
		function(res){
...
function addHoverDom(treeId, treeNode) {
	...
	if (btn) btn.bind("click", function(){
		...
		da.runDB("/sys_admin/module/setting/item/action/itemtype_add_item.php",{
			itpid: treeNode.id,
			name: "新建分类"
		},
		function(res){
...
function additem(){
	...
	daWin({
		width: 600,
		height: 420,
		url: "/sys_admin/module/setting/item/item_add.php?itid="+ g_itid,
		title: "新添加可选项",
		after: function(){
			loadlist();
		}
...
function deleteitem(){
	...
	if( iids ){
		confirm("确认删除选中的可选项吗？",function(){
			da.runDB("/sys_admin/module/setting/item/action/item_delete_list.php",{
				itid: g_itid,
				iids: iids.join(",")
			},function(res){
				if("FALSE" == res){
...
function loadlist(){
	daTable({
		id: "tb_list",
		url: "/sys_admin/module/setting/item/action/item_get_page.php",
		data: {
			dataType: "json",
			// opt: "qry",
			itid: g_itid
...
*/
function loadinfo(){
	da.runDB("/sys_admin/module/setting/item/action/itemtype_get_item.php",{
		dataType: "json",
		itid: g_itid
	},
	function(res){
...
/*加载左边部门数据*/
function loadtree(){
	da.runDB("/sys_admin/module/setting/item/action/itemtype_get_list.php",{
		dataType: "json",
	},
	function(data){
		var zNodes = [];
