...
class Dir{
	...
	*/
	public static function dir_copy($fromdir, $todir) {
		$fromdir = Dir::dir_path($fromdir);
		$todir = Dir::dir_path($todir);
		if (!is_dir($fromdir)) return FALSE;
		if (!is_dir($todir)) Dir::dir_create($todir);
		$list = glob($fromdir.'*');
		if (!empty($list)) {
			foreach($list as $v) {
	...
	*/
	public static function dir_touch($path, $mtime = TIME, $atime = TIME) {
		...
		$files = glob($path.'*');
		foreach($files as $v) {
			is_dir($v) ? Dir::dir_touch($v, $mtime, $atime) : touch($v, $mtime, $atime);
		}
		return true;
	}
	/**
	...
	*/
	public static function dir_delete($dir) {
		...
		$list = glob($dir.'*');
		foreach($list as $v) {
			is_dir($v) ? Dir::dir_delete($v) : @unlink($v);
		}
		return @rmdir($dir);
	}
}
